                              ...
                              __u32 slot)
{
    ...
    {
        struct allow_ip_bpf *obj = allow_ip_bpf__open();
        if (!obj)
        {
            log_err(conf, "fail: opening allow_ip skeleton\n");
            return 1;
        }
        ...
        if (err)
        {
            log_err(conf, "fail: reusing prog_array for allow_ip\n");
            allow_ip_bpf__destroy(obj);
            return 1;
        }
        ...
        if (entry->has_input)
        {
            ...
            if (err)
            {
                log_err(conf, "fail: setting rodata for allow_ip\n");
                allow_ip_bpf__destroy(obj);
                return 1;
            }
        }

        err = allow_ip_bpf__load(obj);
        if (err)
        {
            libbpf_strerror(err, buf, sizeof(buf));
            log_err(conf, "fail: loading allow_ip: %s\n", buf);
            allow_ip_bpf__destroy(obj);
            return 1;
        }

        int prog_fd = bpf_program__fd(obj->progs.allow_ip);
        err = bpf_map_update_elem(prog_array_fd, &slot, &prog_fd, BPF_ANY);
        if (err)
        {
            log_err(conf, "fail: inserting allow_ip into prog_array slot %d\n", slot);
            allow_ip_bpf__destroy(obj);
            return 1;
        }

        log_out(conf, "done: loaded allow_ip at slot %d\n", slot);
